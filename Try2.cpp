...
void solve(){
    ...
    vector<pair<string,int>> per_skills;
    person per1;
    ...
    do{
        ...
        rep(i,0,sz){
                  ...
                  if(vis[j]){
                    ...
                    rep(l,0,per_skills.size()){
                        ...
                        if(temp[k].first==per_skills[l].first && temp[k].second<=per_skills[l].second && !vis[j]){
                            ...
                            if(temp[k].second==per_skills[l].second){
                              inc.pb({per1.name,per_skills[l].first});
                            }
                            DoneBy.pb(per1.name);
                            vis[j]=true;
                            milgya=true;
                            break;
                        }else if(temp[k].first==per_skills[l].first && temp[k].second==per_skills[l].second+1 && !vis[j]){
                          
                          if(mp[temp[k].first]>=temp[k].second){
                            DoneBy.pb(per1.name);
                            vis[j]=true;
                            milgya=true;
                            break;
                    ...
                    if(milgya){
                        rep(l,0,per_skills.size()){
                          mp[per_skills[l].first]=max(mp[per_skills[l].first],per_skills[l].second);
            ...
            if(temp.size()==DoneBy.size()){
                mx_days= max(mx_days,pro1.days);
                ans.pb({pro1.name,DoneBy});

                rep(j,0,inc.size()){
                  bool milgya=false;
                  rep(k,0,per){
                    person per1=conter[k];
                    per_skills = per1.skills;
                    rep(l,0,per_skills.size()){
                      if(per1.name==inc[j].first && per_skills[l].first==inc[j].second){
                        conter[k].skills[l].second++;
                        bool milgya=true;
                        break;
                      }
                    }
